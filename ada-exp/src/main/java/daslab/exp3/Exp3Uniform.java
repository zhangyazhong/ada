...
public class Exp3Uniform extends ExpTemplate {
        ...
        for (int time = 0; time < TIMES; time++) {
            ...
            for (int index = 0; index < EXP_DATA.length; index++) {
                ...
                try {
                    ...
                    getVerdict().execute("CREATE " + SAMPLE_RATIO + "% UNIFORM SAMPLE OF wiki_exp3.pagecounts");
                    ...
                    AdaLogger.info(this, "Data[" + data + "] with uniform ratio[" + SAMPLE_RATIO + "%] cost: " + samplingTime);
    ...
    private void output() {
        for (int i = 0; i < cost.length; i++) {
            ...
            AdaLogger.info(this, String.format("No.%d data scale[%d] with uniform ratio[%d%%] cost: %s", i, EXP_DATA[i], SAMPLE_RATIO, samplingTime));
        }
    }
