...
public class Exp11AdaTimeCost extends ExpTemplate {
    private final static int REPEAT_TIME = 1;
    private static String RESULT_SAVE_PATH;

    public Exp11AdaTimeCost() {
        this("Ada Exp11 - Ada with Adaptive Time Cost on Stratified Sampling");
    }

    public Exp11AdaTimeCost(String name) {
        super(name);
    }

    @Override
    public void run() {
        RESULT_SAVE_PATH = String.format("/tmp/ada/exp/exp11/ada_cost_%d_%d_%d.csv", HOUR_START, HOUR_TOTAL, HOUR_INTERVAL);
        ...
        for (int k = 0; k < REPEAT_TIME; k++) {
            ...
            for (int i = HOUR_START; i < HOUR_TOTAL; i++) {
                ...
                for (Map.Entry<String, Object> entry : executionReport.search("sampling.count").entrySet()) {
                    expResult.push(time, entry.getKey(), String.valueOf(entry.getValue()));
                }
                for (Map.Entry<String, Object> entry : executionReport.search("adaptive").entrySet()) {
                    expResult.push(time, entry.getKey(), String.valueOf(entry.getValue()));
                }
