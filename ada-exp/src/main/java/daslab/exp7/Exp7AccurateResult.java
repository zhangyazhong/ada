...
public class Exp7AccurateResult extends ExpTemplate {
    ...
    public void run() {
        ...
        QUERIES = ImmutableList.of(
                ...
                "SELECT sum(case when p_type like 'PROMO%' then l_extendedprice * (1 - l_discount) else 0 end) * 100.0 / sum(l_extendedprice * (1 - l_discount)) as promo_revenue FROM tpch.lineitem, tpch.part WHERE l_partkey = p_partkey AND l_shipdate >= '1995-09-01' AND l_shipdate < '1995-10-01'",
